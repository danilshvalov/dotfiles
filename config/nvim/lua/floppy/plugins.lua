...
local function _2_(use)
  ...
  do
    ...
    use({config = _8_, "ray-x/lsp_signature.nvim"})
    local function _9_()
      local module_1_auto = require("floppy.configs.leap")
      return module_1_auto.setup()
    end
    use({config = _9_, requires = "tpope/vim-repeat", "ggandor/leap.nvim"})
    local function _10_()
      return require("floppy.configs.virt-column")
    end
    use({config = _10_, "lukas-reineke/virt-column.nvim"})
    local function _11_()
      local module_1_auto = require("fidget")
      return module_1_auto.setup()
    end
    use({config = _11_, "j-hui/fidget.nvim"})
    local function _12_()
      local module_1_auto = require("floppy.configs.null-ls")
      return module_1_auto.setup()
    end
    use({after = "nvim-cmp", config = _12_, "jose-elias-alvarez/null-ls.nvim"})
    local function _13_()
    ...
    use({after = {"github-nvim-theme"}, config = _14_, requires = {"kyazdani42/nvim-web-devicons"}, "nvim-lualine/lualine.nvim"})
    local function _15_()
      local module_1_auto = require("gitsigns")
      return module_1_auto.setup()
    end
    use({config = _15_, requires = {"nvim-lua/plenary.nvim"}, "lewis6991/gitsigns.nvim"})
    local function _16_()
      local module_1_auto = require("floppy.configs.neo-tree")
      return module_1_auto.setup()
    end
    use({branch = "v2.x", config = _16_, requires = {"nvim-lua/plenary.nvim", "kyazdani42/nvim-web-devicons", "MunifTanjim/nui.nvim"}, "nvim-neo-tree/neo-tree.nvim"})
    local function _17_()
      local module_1_auto = require("floppy.configs.todo-comments")
      return module_1_auto.setup()
    end
    use({config = _17_, requires = "nvim-lua/plenary.nvim", "folke/todo-comments.nvim"})
    use({"famiu/bufdelete.nvim"})
    local function _18_()
      local module_1_auto = require("floppy.configs.move")
      return module_1_auto.setup()
    end
    use({config = _18_, "fedepujol/move.nvim"})
    local function _19_()
      local module_1_auto = require("floppy.configs.vimtex")
      return module_1_auto.setup()
    end
    use({config = _19_, ft = "tex", "lervag/vimtex"})
    local function _20_()
      local module_1_auto = require("floppy.configs.bufferline")
      return module_1_auto.setup()
    end
    use({config = _20_, requires = "kyazdani42/nvim-web-devicons", "akinsho/bufferline.nvim"})
    local function _21_()
      local module_1_auto = require("floppy.configs.nvim-autopairs")
      return module_1_auto.setup()
    end
    use({config = _21_, "windwp/nvim-autopairs"})
    use({"tpope/vim-surround"})
    local function _22_()
      local kommentary = require("kommentary.config")
      return kommentary.configure_language("default", {prefer_single_line_comments = true})
    end
    use({config = _22_, "b3nj5m1n/kommentary"})
    local function _23_()
      local module_1_auto = require("floppy.configs.lspconfig")
      return module_1_auto.setup()
    end
    use({config = _23_, requires = "williamboman/nvim-lsp-installer", "neovim/nvim-lspconfig"})
    local function _24_()
      local module_1_auto = require("floppy.configs.nvim-cmp")
      return module_1_auto.setup()
    end
    use({config = _24_, requires = {"hrsh7th/cmp-nvim-lsp", "hrsh7th/cmp-buffer", "hrsh7th/cmp-path", "hrsh7th/cmp-cmdline"}, "hrsh7th/nvim-cmp"})
    local function _25_()
      local module_1_auto = require("floppy.configs.lsp-colors")
      return module_1_auto.setup()
    end
    use({config = _25_, "folke/lsp-colors.nvim"})
    local function _26_()
      local module_1_auto = require("floppy.configs.lspsaga")
      return module_1_auto.setup()
    end
    use({config = _26_, "tami5/lspsaga.nvim"})
    local function _27_()
      local module_1_auto = require("floppy.configs.telescope")
      return module_1_auto.setup()
    end
    use({config = _27_, requires = {"nvim-lua/popup.nvim", "nvim-lua/plenary.nvim", "nvim-telescope/telescope-project.nvim", "tami5/sqlite.lua"}, "nvim-telescope/telescope.nvim"})
    local function _28_()
      local module_1_auto = require("floppy.configs.nvim-treesitter")
      return module_1_auto.setup()
    end
    use({config = _28_, run = ":TSUpdate", "nvim-treesitter/nvim-treesitter"})
    local function _29_()
      local module_1_auto = require("floppy.configs.lsp-trouble")
      return module_1_auto.setup()
    end
    use({config = _29_, "folke/lsp-trouble.nvim"})
    local function _30_()
      local module_1_auto = require("floppy.configs.which-key")
      return module_1_auto.setup()
    end
    use({config = _30_, "folke/which-key.nvim"})
    use({"kburdett/vim-nuuid"})
    local function _31_()
      local module_1_auto = require("floppy.configs.indent-blankline")
      return module_1_auto.setup()
    end
    use({config = _31_, "lukas-reineke/indent-blankline.nvim"})
    ...
    use({config = _32_, "projekt0n/github-nvim-theme"})
    local function _33_()
      return require("impatient")
    end
    use({config = _33_, "lewis6991/impatient.nvim"})
  end
