...
return {
  ...
  },
  {
    ...
    config = function()
      ...
      map
        :prefix("g", "+goto")
        :set("d", vim.lsp.buf.definition, { desc = "Go definitions" })
        :set("D", vim.lsp.buf.declaration, { desc = "Go declaration" })
        :set("r", vim.lsp.buf.references, { desc = "Go references" })
        :set("i", vim.lsp.buf.implementation, { desc = "Go implementations" })
  ...
  },
  {
    ...
    config = function()
      ...
      treesitter.setup({
        ...
        indent = {
          enable = true,
          disable = { "python", "java", "yaml", "sql", "latex", "markdown", "xml" },
        },
        autopairs = {
          enable = true,
        },
  ...
  },
  {
    ...
    config = function()
      ...
      local clang_format
      if false and vim.fn.executable("taxi-format") == 1 then
        clang_format = "taxi-format"
      else
      ...
      conform.setup({
        ...
        formatters_by_ft = {
          lua = { "stylua" },
          python = function()
            if vim.startswith(vim.fn.expand("%:~"), "~/arcadia") then
              return { "taxi_format" }
            else
              return { "black" }
            end
    ...
    config = function()
      ...
      local function is_cpp_class(opts)
        return vim.bo.filetype == "cpp"
          and (opts.line:match("struct%s+") ~= nil or opts.line:match("class%s+") ~= nil)
      end
    ...
    config = function()
      ...
      require("tokyonight").setup({
        ...
        on_highlights = function(highlights, colors)
          ...
          highlights["@markup.strong.markdown_inline"] = { fg = colors.orange, bold = true }
        end,
      })
  ...
  },
  {
    ...
    config = function()
      ...
      map
        :prefix("<leader>o", "Obsidian")
        :set("t", function()
          vim.cmd.edit("~/obsidian/список-задач.md")
        end, { desc = "Files" })
        :set("f", function()
          ...
          vim
            .system({
              ...
              string.format(
                ...
                "notes",
                vim.fs.relpath(
                  "/Users/danilshvalov/projects/obsidian/notes",
      ...
      map
        ...
        :set("G", function()
          ...
          if cwd ~= nil then
            return fzf.live_grep({ cwd = cwd })
          end
        end, { desc = "Grep files in directory" })
      ...
      fzf.setup({
        ...
        files = {
          fd_opts = "--color=never --type f --hidden --follow --exclude .git --exclude .obsidian --exclude build --exclude .DS_Store --exclude Вложения --exclude .mypy_cache --exclude __pycache__",
          fzf_opts = {
  ...
  },
  {
    ...
    config = function()
      ...
      -- local function find_char_pos_2(s, init)
      --   if not init then
      --     return
      --   end
  ...
  --     patterns.TagCharsOptional = "[%w%d_/-]*"
  --     patterns.TagCharsRequired = "[%w]+[%w%d_/-]*[%w%d]+"
  --     patterns.Tag = "#[%w]+[%w%d_/-]*[%w%d]+"
  --     patterns.NakedUrl = "https?://[%w%d._-]+[%w%d._#/=&?:+%%-]+[%w%d/]"
  ...
  },
  {
    ...
    keymap = function(map)
      map:prefix("<leader>c", "Conflicts"):set("t", vim.cmd.ConflictMarkerThemselves)
    end,
  },
  {
    "folke/zen-mode.nvim",
  ...
  },
  {
    ...
    config = function()
      ...
      splits.setup({ default_amount = 1 })

      map:set("<A-h>", splits.resize_left)
      map:set("<A-j>", splits.resize_down)
      map:set("<A-k>", splits.resize_up)
      map:set("<A-l>", splits.resize_right)
    end,
  },
}
