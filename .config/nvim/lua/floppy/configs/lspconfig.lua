...
local function setup_clangd()
  return lspconfig.clangd.setup({cmd = {"clangd", "--background-index", "--compile-commands-dir", "build"}, init_options = {compilationDatabasePath = "build"}, root_dir = util.root_pattern("build/compile_commands.json"), ["capabilities.offsetEncoding"] = {"utf-16"}, on_attach = disable_formatting})
end
...
local function setup_servers()
  lspinstaller.setup({ensure_installed = {"clangd", "jdtls", "rust_analyzer", "sumneko_lua", "texlab", "pyright"}})
  setup_clangd()
  setup_jdtls()
  setup_rust_analyzer()
  setup_sumneko_lua()
