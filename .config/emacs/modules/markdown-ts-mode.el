;;; markdown-ts-mode.el --- Major mode for Markdown-formatted text -*- lexical-binding: t; -*-

;; Copyright (C) 2007-2023 Jason R. Blevins and markdown-ts-mode
...
;; URL: https://jblevins.org/projects/markdown-ts-mode/
...
(defconst markdown-ts-mode-version "2.7-alpha"
...
(defgroup markdown nil
  ...
  :prefix "markdown-"
  :group 'text
  :link '(url-link "https://jblevins.org/projects/markdown-ts-mode/"))
...
same margin."
  ...
  :type 'boolean
  :safe 'booleanp
  :package-version '(markdown-ts-mode . "2.4"))
...
function or \\[markdown-toggle-wiki-links]."
  ...
  :type 'boolean
  :safe 'booleanp
  :package-version '(markdown-ts-mode . "2.2"))
...
project: search for wiki link targets under project root"
  ...
  :package-version '(markdown-ts-mode . "2.5"))
...
be used."
  :type '(choice
          ...
          (repeat :tag "List of possible URL replacement characters"
                  character))
  :package-version '(markdown-ts-mode . "2.3"))
...
\\<markdown-ts-mode-map>\\[markdown-follow-thing-at-point] and \\[markdown-follow-link-at-point]
...
This variable must be set before loading markdown-ts-mode."
  :group 'markdown
  :type 'boolean
  :safe 'booleanp
  :package-version '(markdown-ts-mode . "2.5"))
...
(defvar markdown-literal-faces
  ...
  "A list of markdown-ts-mode faces that contain literal text.
...
(defun markdown-syntax-propertize-pre-blocks (start end)
  ...
  (save-excursion
    ...
    (let (finish)
      ...
      ;; https://github.com/jrblevin/markdown-ts-mode/issues/512
      (while (not finish)
...
(defun markdown-syntax-propertize-comments (start end)
  ...
  ;; https://github.com/jrblevin/markdown-ts-mode/issues/536
  (let (finish)
    (goto-char start)
    (while (not finish)
...
See `markdown-hide-markup' for additional details."
  ...
  (if markdown-hide-markup
      (progn (add-to-invisibility-spec 'markdown-markup)
             (message "markdown-ts-mode markup hiding enabled"))
    (progn (remove-from-invisibility-spec 'markdown-markup)
           (message "markdown-ts-mode markup hiding disabled")))
...
Used for `flyspell-generic-check-word-predicate'."
  (save-excursion
    (goto-char (1- (point)))
    ;; https://github.com/jrblevin/markdown-ts-mode/issues/560
...
(defvar markdown-ts-mode-syntax-table
  (let ((tab (make-syntax-table text-mode-syntax-table)))
    (modify-syntax-entry ?\" "." tab)
    tab)
  "Syntax table for `markdown-ts-mode'.")
...
(defvar markdown-ts-mode-style-map
...
(defvar markdown-ts-mode-command-map
...
(defvar markdown-ts-mode-map
  (let ((map (make-keymap)))
    ...
    (define-key map (kbd "C-c C-s") markdown-ts-mode-style-map)
    ...
    (define-key map (kbd "C-c C-c") markdown-ts-mode-command-map)
...
(defvar markdown-ts-mode-mouse-map
...
(easy-menu-define markdown-ts-mode-menu markdown-ts-mode-map
  ...
  '("Markdown"
    ...
    ("Documentation"
     ...
     ["Homepage" markdown-ts-mode-info]
     ["Describe Mode" (describe-function 'markdown-ts-mode)]
     ["Guide" (browse-url "https://leanpub.com/markdown-ts-mode")])))
...
(defun-markdown-ref-checker
  markdown-check-refs
  "Show all undefined Markdown references in current `markdown-ts-mode' buffer.
...
(defun-markdown-ref-checker
  ...
  "Show all unused Markdown references in current `markdown-ts-mode' buffer."
...
(defun markdown-live-preview-remove-on-kill ()
  (cond ((and (derived-mode-p 'markdown-ts-mode)
              markdown-live-preview-mode)
...
(defun markdown-fontify-inline-links (last)
  ...
  (when (markdown-match-generic-links last nil)
    (let* ((link-start (match-beginning 3))
           ...
           (lp (list 'keymap markdown-ts-mode-mouse-map
                     'mouse-face 'markdown-highlight-face
                     'font-lock-multiline t
                     'help-echo (if title (concat title "\n" url) url)))
           ;; URL part
           (up (list 'keymap markdown-ts-mode-mouse-map
                     'invisible 'markdown-markup
                     'mouse-face 'markdown-highlight-face
                     'font-lock-multiline t))
...
(defun markdown-fontify-plain-uris (last)
  ...
  (when (markdown-match-plain-uris last)
    (let* ((start (match-beginning 0))
           (end (match-end 0))
           (props (list 'keymap markdown-ts-mode-mouse-map
                        'face 'markdown-plain-url-face
                        'mouse-face 'markdown-highlight-face
                        'rear-nonsticky t
                        'font-lock-multiline t)))
...
and disable it otherwise."
  ...
  (if markdown-hide-urls
      (message "markdown-ts-mode URL hiding enabled")
    (message "markdown-ts-mode URL hiding disabled"))
  (markdown-reload-extensions))
...
the new buffer remains in `markdown-ts-mode'.  Open the link in another
...
and disable it otherwise."
  ...
  (if markdown-enable-wiki-links
      (message "markdown-ts-mode wiki link support enabled")
...
This is a `fill-forward-paragraph-function' for `markdown-ts-mode'.
...
Checks to see if there is actually a ‘markdown-ts-mode’ file local variable
...
(defconst markdown-ts-mode-font-lock-keywords-math
  (list
  ...
  (if markdown-enable-math
      (progn
        (font-lock-add-keywords
         'markdown-ts-mode markdown-ts-mode-font-lock-keywords-math)
        (message "markdown-ts-mode math support enabled"))
    (font-lock-remove-keywords
     'markdown-ts-mode markdown-ts-mode-font-lock-keywords-math)
    (message "markdown-ts-mode math support disabled"))
  (markdown-reload-extensions))
...
(defun markdown--get-remote-image (url)
  ...
  (or (gethash url markdown--remote-image-cache)
      (let ((dl-path (make-temp-file "markdown-ts-mode--image")))
        (require 'url)
...
and disable otherwise."
  ...
  (if markdown-fontify-code-blocks-natively
      (message "markdown-ts-mode native code block fontification enabled")
    (message "markdown-ts-mode native code block fontification disabled"))
  (markdown-reload-extensions))
...
(defun markdown-show-version ()
  ...
  (interactive)
  (message "markdown-ts-mode, version %s" markdown-ts-mode-version))

(defun markdown-ts-mode-info ()
  "Open the `markdown-ts-mode' homepage."
  (interactive)
...
(define-derived-mode markdown-ts-mode text-mode "Markdown"
  ...
  (setq-local treesit-font-lock-feature-list
              '((comment definition preprocessor)
                (keyword string type header emphasis link)
                (function constant markup inline-code)
...
;;;###autoload
(define-derived-mode gfm-mode markdown-ts-mode "GFM"
...
;;;###autoload
(define-derived-mode markdown-view-mode markdown-ts-mode "Markdown-View"
